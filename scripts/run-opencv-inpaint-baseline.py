...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run a fixed-rectangle OpenCV inpainting baseline on a video."
    )
...
def main() -> int:
    ...
    if args.x + args.width > width or args.y + args.height > height:
    ...
    mask[args.y : args.y + args.height, args.x : args.x + args.width] = 255

    margin = max(8, math.ceil(args.radius * 3))
    crop_x0 = max(0, args.x - margin)
    crop_y0 = max(0, args.y - margin)
    crop_x1 = min(width, args.x + args.width + margin)
    crop_y1 = min(height, args.y + args.height + margin)
    ...
    try:
        while True:
            ...
            frame[
                ...
                args.y - crop_y0 : args.y - crop_y0 + args.height,
                args.x - crop_x0 : args.x - crop_x0 + args.width,
            ]
